...
// Interfaces corregidas según la estructura real
interface Cliente {
...
class SameepDataCollectorCorregido {
  private browser: any;
  private context: any;
  private page: any;
          ...
          if (suministroEspecifico) {
            ...
            if (match && match[1] === suministroEspecifico.numeroSuministro) {
              ...
              if (comprobante.tienePdf) {
                // En la fase 1, solo marcamos que tiene PDF pero no lo descargamos
                console.log(`📄 ${comprobante.numeroFactura} - CON PDF (se descargará en Fase 2)`);
  ...
  async procesarClienteConReintentos(cliente: Cliente, intento: number = 1): Promise<void> {
    try {
      ...
      if (intento < this.MAX_REINTENTOS) {
        ...
        await this.tomarScreenshot(`error_final_cliente_${cliente.id}`);
      }
    }
  }
...
export { SameepDataCollectorCorregido };
